"AnPhenoMetrics" <-
function (TS, outfile, outgraph, Ystart, period, SOSth = 0.5, 
    EOSth = 0.5) 
{
    if (outgraph != FALSE) {
        pdf(outgraph)
    ...
    for (j in 1:length(TS[, 1])) {
        ...
        plot(ts(TS[j, ], start = Ystart, freq = period), ylim = c(lim, 
            liM), type = "l", xlab = "time", ylab = "NDVI", main = paste(rownames(TS)[j], 
            "NDVI Time Series"))
        year = c()
        n = 1
        ...
        if (length(meanM == 1) & mean(meanC) > 0.2 & mean(meanC) < 
            0.7) {
            Mdm = meanM
            mdm = meanm
            Mm = max(meanC)
        ...
        if (length(meanM == 2) & mean(meanC) > 0.2 & mean(meanC) < 
            0.7) {
