pointtobuffer=function(shapefile,shapedir,ndvidirectory,region,Ystart,nameshape="buffer",dirshape=".",rad=1)
{
...
for (y in -rad:rad)
{
coord=rbind(coord,coordinates(inPoints)[i,]+c(x*gridparameters(inGrid)[1,2],y*gridparameters(inGrid)[2,2]))
data=rbind(data,as.data.frame(inPoints[names(inPoints)])[i,])
}
}
}
names(data)=substr(names(data),1,10)
res=SpatialPointsDataFrame(coords=coord,proj4string=CRS(proj4string(inPoints)),data=as.data.frame(data))
#saving the new shapefile with only the region of interest
